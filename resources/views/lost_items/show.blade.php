            ...
            <div class="col-md-4">
                <div class="border rounded mb-3" style="height:260px;">
                    <img src="{{ $item->image ? asset('storage/' . $item->image) : 'https://via.placeholder.com/400x260?text=No+Image' }}"
                        class="w-100 h-100 rounded" style="object-fit:cover;" alt="Gambar Barang">
                </div>
            </div>
            ...
            <div class="col-md-8">
                <div class="bg-white rounded shadow-sm">
                    <ul class="list-group list-group-flush">
                        ...
                        <li class="list-group-item"><strong>Kategori:</strong>
                            {{ $item->kategori ?? '-' }}</li>
                        <li class="list-group-item"><strong>Lokasi Terakhir:</strong>
                            {{ $item->lokasi_terakhir ?? '-' }}</li>
                        <li class="list-group-item"><strong>Tanggal Kehilangan:</strong>
                            {{ $item->tanggal_hilang ? $item->tanggal_hilang->format('d-m-Y') : '-' }}</li>
                        <li class="list-group-item"><strong>Kontak:</strong> {{ $item->kontak ?? '-' }}</li>
                        <li class="list-group-item"><strong>Deskripsi:</strong>
                            <div class="mt-1 text-muted">{{ $item->deskripsi ?? '-' }}</div>
                        </li>
                    </ul>
                </div>
                ...
                <div class="mt-3 d-flex gap-2">
                    ...
                    @can('admin-only')
                        ...
                        <form action="{{ route('lost-items.destroy', $item->id) }}" method="POST"
